---
title: "Combination_all_Yerly_BCCs"
Author: "Laura Yerly"
...
```{r}
#matrix_dir = "M:/DER/LABOS/ONCODER/Laura/1-Experiments/4-Bioinformatic/2-Human BCC scRNAseq (Yao_Yost_Yerly)/10-BCC7LY_GTF_CellRanger/LY021121/filtered_feature_bc_matrix/"

matrix_dir = "~/Dropbox/CSI/Collaborations/Kuonen_BCC/data/raw/CellRanger_LY/LY021121_filtered_feature_bc_matrix/"
...
name, add gene names as rownames

```{r}
read_mtx <- function(matrix_dir, samplename) {
  # matrix_dir is the full path to where the cell ranger output files are stored
  # samplename is a character to add to the cell names to track each cell from which sample it comes from.
...
names(matrices) <- ids
```
Problematic gene names, where dashes were substituted with dots
```{r}
library(STACAS)
...
```{r eval=F}
library(STACAS)
obj.list <- SplitObject(bcc_7LY, split.by = "Sample")

data_integrated <- Run.STACAS(obj.list, cell.labels = "scGate_multi")
```

```{r eval=F fig.width=12}
data_integrated <- RunUMAP(data_integrated, reduction = "pca", dims = 1:30)

a <- DimPlot(data_integrated, reduction = "umap", group.by = "Sample") + theme(aspect.ratio = 1)
b <- DimPlot(data_integrated, group.by = "scGate_multi", label = T, repel = T, label.size = 2) +
    theme(aspect.ratio = 1) + ggtitle("scGate annotation")
a | b
```
